 ...
 */
export const VALID_PLAN_TIERS: PlanTier[] = ["free", "pro", "elite"];
...
export type TierValidationResult =
  ...
  | { success: false; error: UnknownTierError };
...
export function isValidPlanTier(tier: string): tier is PlanTier {
  return VALID_PLAN_TIERS.includes(tier as PlanTier);
}
 ...
 */
export function validatePlanTier(tier: string, context: string): TierValidationResult {
 ...
 */
export function assertValidPlanTier(tier: string, context: string): asserts tier is PlanTier {
  if (!isValidPlanTier(tier)) {
...
/**
 * Legacy alias mapping — DISABLED in production (fail-closed hardening).
 * Kept for documentation; set ALLOW_LEGACY_PLAN_ALIASES = true only in
 * a one-time data-migration context, never at runtime.
 */
 ...
 * Legacy aliases are BLOCKED by default (fail-closed).
 * Returns null for unknown tiers (caller must handle explicitly).
 */
export function normalizeToPlanTier(plan: string | null | undefined): PlanTier | null {
  if (!plan) return "free";
...
export interface PlanEntitlement {
  ...
  /** Whether this is a paid tier (pro, elite). Structural — not admin-configurable. */
  isPaid: boolean;
  /** Whether token usage draws from a paid allocation bucket. Structural — not admin-configurable. */
...
export type EntitlementResolutionResult =
  | { success: true; plan: PlanTier; entitlements: PlanEntitlement; source: "admin" | "defaults" }
  | { success: false; error: UnknownTierError };
 ...
 */
export type UnknownTierPolicy = 
  | "error"      // Throw UnknownTierError (use in logic paths)
  | "restrict"   // Return restricted entitlements (use in enforcement)
  | "allow";     // Allow but log warning (use in display paths only)
