/**
 * POST /api/execution/trigger/suppressed
 * Logs a trigger_suppressed event to behaviour_events.
...
export async function POST(req: NextRequest) {
  ...
  try {
    await rateLimit({ key: `user:trigger:suppressed:${userId}`, limit: RATE.limit, window: RATE.window });
  } catch (err: unknown) {
  ...
  const parsed = TriggerSuppressedSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const { data: existing } = await fromSafe("behaviour_events")
      .select("id")
      .eq("user_id", userId)
      .eq("event_type", "trigger_suppressed")
  ...
  // Log trigger_suppressed event — subject_code null, everything in metadata
