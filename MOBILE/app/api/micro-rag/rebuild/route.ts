...
import { z } from "zod";
import { buildMicroRagCache } from "@/lib/fast/cacheBuilder";
import { saveCache } from "@/lib/fast/cacheStore";
import {
...
export async function POST(req: NextRequest) {
  const protectionResponse = await enforceServiceKeyProtection(req, "micro_rag");
  ...
  try {
    ...
    const cache = await buildMicroRagCache(userId);
    ...
    safeErrorLog("[api/micro-rag/rebuild] error", error);
    return NextResponse.json({ error: "cache_rebuild_failed" }, { status: 500 });
  }
}
