-- Migration: Phase 6B — Durable raw behavioural data (journal, check-ins, conversation).
-- Server is authoritative; no localStorage as source of truth for these domains.
...
COMMENT ON TABLE public.journal_entries IS 'Phase 6B: durable journal entries; server authoritative.';
COMMENT ON TABLE public.check_ins IS 'Phase 6B: durable check-ins (mood, stress, energy, focus, note).';
COMMENT ON TABLE public.conversation_messages IS 'Phase 6B: durable user/assistant messages for Vella conversation.';
