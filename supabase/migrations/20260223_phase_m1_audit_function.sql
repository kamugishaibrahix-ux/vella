-- Phase M1: RPC that runs the audit (metadata only). Same logic as runbook-sql/phase_m1_audit.sql.
...
COMMENT ON FUNCTION public.run_phase_m1_audit() IS 'Phase M1: aggregate counts and byte estimates only. No user text.';

REVOKE EXECUTE ON FUNCTION public.run_phase_m1_audit() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.run_phase_m1_audit() FROM anon;
REVOKE EXECUTE ON FUNCTION public.run_phase_m1_audit() FROM authenticated;
