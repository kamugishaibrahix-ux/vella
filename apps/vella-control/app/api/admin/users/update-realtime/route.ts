...
export async function POST(request: Request) {
  ...
  try {
    ...
    await rateLimitAdmin(request, "users-update-realtime", userId);
  } catch (err: unknown) {
  ...
  try {
    ...
    const { error: logError } = await supabase.from("admin_activity_log").insert({
      ...
      action: "users.update-realtime",
      previous: { realtime_beta: existing.realtime_beta },
      next: { realtime_beta: payload.enabled },
    });
    ...
    console.error(error);
    let message = "Failed to update realtime beta.";
